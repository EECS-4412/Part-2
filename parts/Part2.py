...
def part2():
    ...
    with open(f'csv/215659501-215528797-allen-T2-2.csv', 'w') as f:
        ...
        for row in rows:
            w.writerow(row)
            birthdates.append(datetime.strptime(row[0].split("T")[0], "%Y-%m-%d")) 

    average_date = datetime.utcfromtimestamp(sum([datetime.timestamp(x) for x in birthdates])//len(birthdates))
    ...
    with open(f'csv/215659501-215528797-allen-T2-4.csv', 'w') as f:
        ...
        for pts_home, pts_away, team_name_home, team_name_away, game_date in rows:
            w = csv.writer(f,delimiter=',')
            w.writerow((pts_home, pts_away, team_name_home, team_name_away, game_date))
            points_per_game.append(pts_home if "Toronto" in team_name_home else pts_away)
